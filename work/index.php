...
$dir="../";
if(isset($_GET['title'])){
    $my_work_url = $_GET['title'];
}
else{
    $my_work_url = $_GET['value'];
}

$work_blog = "SELECT * FROM `works_db` WHERE `canonical_url`='$my_work_url'";
$res_work = mysqli_query($link, $work_blog);
$row_work = mysqli_fetch_assoc($res_work);

$the_email = $row_work['email_id'];
