...
$query_user_email = "SELECT * FROM `works_db` WHERE `canonical_url`='$blog_id'";
...
$blog_title = $row_blogs['work_title'];
$last_updated = $row_blogs['last_updated'];
$date_created = $row_blogs['added_date'];
$blog_id = $row_blogs['work_id'];
$title = $row_email['work_title'];
$blog_content = $row_blogs['work_description'];
...
<!--Share Buttons-->
<div class="container rounded-4" style="min-height: 80vh; background-color:<?php echo $bg_color ?>">
    <div class="row mb-4" >
        <div class="col-lg-9 p-4 mb-4" style="background-color: <?php echo $bg_color ?>;" align="center">
            <div class="col-lg-9 col-12" align="left" >
                <?php
                echo '<h1 style="color:'.$text_color.'">'.$row_blogs['work_title'].'</h1></span>';?>
                <?php
                echo '<br>';
                echo '<div class="fr-view justify-content-start" id="richViewer" style="color:'.$text_color.'">'.$row_blogs['work_description'].'</div>';
        ...
        <div class="col-lg-3" style="border-left: 0px solid rgb(100,100,100);">
            <div class="sticky-top">
                <?php
                $query_blogs = "SELECT * FROM `works_db` WHERE `email_id`='$userEmailID' AND `work_id`!='$blog_id' ORDER BY RAND() LIMIT 7";
                ...
                while($row_blogs = mysqli_fetch_assoc($res_blogs)){
                    ...
                    <a href="<?php echo str_replace('#directory#', 'work', $custom_url ) ?>/<?php echo $row_blogs['canonical_url'] ?>")">
                    <div class="row p-2 mb-2 rounded-4 shadow-1">
                        <div class="col-lg-8 col-6">
                            <p class="card-text" style="color:<?php echo $text_color ?>"><?php echo $blog_title ?></p>
                        </div>
                        <div class="col-lg-4 col-6">
                            <img style="width:100px; height: 100px; border-radius: 100%; object-fit: cover" src="https://wooble.org/app/work/upload/<?php echo $row_blogs['cover_pic'] ?>" onerror="this.onerror=null; this.src='<?php echo $post_image ?>"  alt="<?php echo $blog_title ?>"/>
                        </div>
